...
const initializeQueues = async () => {
    try {
        ...
        // Initialize queues after Redis connection is established
        const taskQueueService = new TaskQueueService();
        const reminderQueueService = new ReminderQueueService();
        const notificationModule = new NotificationModule();
        
        return {
            taskQueue: taskQueueService.queue,
            reminderQueue: reminderQueueService.queue,
            notificationQueue: notificationModule.notificationQueue
        };
    } catch (error) {
...
(async () => {
    try {
        ...
        // Set up queue processors here using queues.taskQueue, queues.reminderQueue, etc.
        
        // Process jobs from taskQueue
        queues.taskQueue.process(async (job) => {
        ...
        queues.taskQueue.on('completed', async (job) => {
            try {
        ...
        queues.taskQueue.on('failed', async (job, error) => {
        ...
        queues.reminderQueue.on('completed', async (job) => {
            try {
        ...
        queues.reminderQueue.on('failed', async (job, error) => {
        ...
        queues.notificationQueue.on('completed', async (job) => {
            try {
        ...
        queues.notificationQueue.on('failed', async (job, error) => {
