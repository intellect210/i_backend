const { ReminderQueueService } = require('./bullService');
...
class SchedulerService {
    constructor() {
        this.bullService = new ReminderQueueService();
    }
    ...
    async scheduleReminder(userId, task, options) {
        try {
            ...
            logger.debug(
                `[DEBUG: SchedulerService] jobResult after createJob: ${JSON.stringify(
                    jobResult
                )}`
            );
