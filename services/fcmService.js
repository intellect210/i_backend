...
class FCMService {
  ...
  constructor() {
    try {
      ...
      this.messaging = admin.messaging(FCMService.firebaseApp);
    } catch (error) {
   ...
   async constructNotificationObject(title, body, data = {}) {
    if (!title || !body) {
  ...
  async sendFcmNotification(fcmToken, notification) {
    ...
    try {
      const response = await this.messaging.sendToDevice(fcmToken, notification);
