...
handleMessage: async (ws, message) => {
    ...
    try {
      ...
      ,Model personalised Name: ${
        ...
        // Fetch the followupsPreference
        // const followupsPreference = await preferenceManager.getPreference(userId, 'followupsPreference');
        const followupsPreference = true

        if (followupsPreference) {
            // If followupsPreference is true, attempt to schedule reminder
            try {
                const reminderResponse = await botController.sendMessageWithInstructionsWithStructure(
                    `Current user message: ${text}.`,
                    'tasksClassificationInstruction',
                    null,
                    ...
                    [...history,
                      {
                          role: 'user',
                          parts: [{ text: `Current tasks classification: ${JSON.stringify(remindersStructure)}` }],
                      },
                      {
                          role: 'model',
                          parts: [{ text: 'Understood. I will try to classify the task details.' }],
                      },
                  ]
                );
                

                // console.log("tasksClassificationInstruction result:", reminderResponse)

                if (reminderResponse) {
                    const scheduleResult = await schedulerService.scheduleReminder(userId, reminderResponse);
                    console.log("scheduleResult:", scheduleResult)

                    if (scheduleResult.success) {
                      const finalContext = `Reminder scheduled successfully with job ID: ${scheduleResult.reminderId}. User query: ${text}.`;
                      ...
                      return;
                          ...
                          const finalContext = `Reminder scheduling failed, due to: ${scheduleResult.message}. User query: ${text}.`;
                            await agentStateManager.setState(
                                userId,
                                agentStateManager.states.awaitingBotResponse,
                                messageId
                ...
                console.error('Error processing reminder with followupsPreference:', error);
                  await agentStateManager.setState(
                      userId,
                      agentStateManager.states.error,
                      messageId,
                      'Error during reminder processing.'
                  );
                  websocketService.sendMessage(userId, {
                      type: 'error',
                      code: 'REMINDER_PROCESS_ERROR',
                      message: 'An error occurred during reminder processing.',
                  });
                   return;
            }
        }
      ...
      if (classificationResult.payload.classification === 'no_action_needed') {
          ...
          try {
              ...
              const actionResult = await actionExecutor.executeAction(
                  classificationResult.payload.actionType,
