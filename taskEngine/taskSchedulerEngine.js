        ...
        try {
           const jobResult =  await this.bullService.createJob(
            ...
            if(!jobResult.success) {
                 console.log(`[TaskSchedulerImpl] Failed to enqueue plan to bullmq for queue: ${queueKey} due to ${jobResult.message}`);
                  throw new Error(`Failed to enqueue plan to queue: ${jobResult.message}`);
             }
             console.log(`[TaskSchedulerImpl] Plan enqueued to bullmq successfully, Bull internal job id: ${jobResult.bullInternalJobId} for user ${userId}, scheduleId: ${scheduleId}`);
           return { success: true, message: 'Plan enqueued to bullmq successfully', scheduleId, bullInternalJobId: jobResult.bullInternalJobId, plan };
        } catch (error) {
            console.error(`[TaskSchedulerImpl] Error enqueuing plan to bullmq for queue ${queueKey} for user ${userId}, scheduleId: ${scheduleId}:`, error);
        ...
        try {
           ...
           const enqueueStatus = await this.taskSchedulerImpl.enqueuePlan(userId, plan);
            if(!enqueueStatus.success) {
                console.log(`[TaskScheduler] Failed to schedule task for user ${userId}, query: ${query} due to enqueue plan error ${enqueueStatus.message}`);
                  return { success: false, message: `Failed to schedule task due to enqueue plan error ${enqueueStatus.message}` };
              }
            console.log(`[TaskScheduler] Task scheduled successfully for user ${userId}, query: ${query}`);
            return { success: true, message: 'Task scheduled successfully', scheduleId: enqueueStatus.scheduleId,  bullInternalJobId: enqueueStatus.bullInternalJobId, plan: enqueueStatus.plan };
        } catch (error) {
